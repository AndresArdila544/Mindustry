...
public class HudFragment{
    ...
    private void buildGutterAreas(Group parent){
        //menu at top left
        parent.fill(cont -> {
            ...
            cont.update(() -> {
                ...
                if(Core.input.keyTap(Binding.skipWave) && statusDisplayBuilder.canSkipWave()){
            ...
            wavesMain.table(s -> {
                ...
                s.add(statusDisplayBuilder.makeStatusTable()).grow().name("status");
                ...
                }).growY().fillX().right().width(40f).disabled(b -> !statusDisplayBuilder.canSkipWave()).name("skip").get().toBack();
            }).width(dsize * 5 + 4f).name("statustable");

            wavesMain.row();

            statusDisplayBuilder.addInfoTable(wavesMain.table().width(dsize * 5f + 4f).left().get());
    ...
    private void buildCoreInfo(Group parent){
        //core info
        parent.fill(t -> {
            t.top();
